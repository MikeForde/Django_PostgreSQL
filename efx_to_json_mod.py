#!/usr/bin/env python3
import sys, json, re
from pathlib import Path

TAG_LINE = re.compile(r'^\s*<([A-Za-z0-9_\/]+)>\s*(.*)\s*$')

INT_FIELDS   = {"X","Y","H","W","TAB","CTYPE","PAGEW","PAGEH","FCOL","BCOL","ZINDEX"}
FLOAT_FIELDS = {"FSIZE","FSIZE_F"}
BOOL_FIELDS  = {"DISPLAY","READONLY","VISIBLE"}

def coerce(key, val):
    if val is None: return None
    sval = str(val).strip()
    # Strip dangling "<" some lines have, e.g. "<H>23<"
    if sval.endswith("<"):
        sval = sval[:-1].rstrip()

    # booleans
    if key in BOOL_FIELDS:
        low = sval.lower()
        if low in ("true","false"):
            return low == "true"
        return sval  # fall back to string if odd

    # ints
    if key in INT_FIELDS:
        try:
            return int(float(sval))
        except:
            return None

    # floats
    if key in FLOAT_FIELDS:
        try:
            return float(sval)
        except:
            return None

    # everything else: raw string (preserve &lt;br&gt; etc.)
    return sval

def parse_blocks(lines, start_tag):
    """Yield dicts for blocks that start with <start_tag> and end before next <start_tag>."""
    i, n = 0, len(lines)
    while i < n:
        if lines[i].strip().startswith(f"<{start_tag}>"):
            i += 1
            block = {}
            last_key = None
            while i < n and not lines[i].strip().startswith(f"<{start_tag}>"):
                line = lines[i].rstrip("\n")
                m = TAG_LINE.match(line)
                if m:
                    key, raw = m.group(1), m.group(2)
                    val = coerce(key, raw)
                    block[key] = val
                    last_key = key
                else:
                    # Continuation of previous TEXT line(s) (no opening tag at start)
                    if last_key == "TEXT":
                        block["TEXT"] = (block.get("TEXT","") + ("\n" if block.get("TEXT") else "") + line.strip())
                i += 1
            if block:
                yield block
        else:
            i += 1

def parse_meta(lines):
    meta = {}
    # TFORMS block: FONTNAME_F, FSIZE_F
    for b in parse_blocks(lines, "TFORMS"):
        if "FONTNAME_F" in b: meta["FONTNAME_F"] = b["FONTNAME_F"]
        if "FSIZE_F"   in b: meta["FSIZE_F"]   = coerce("FSIZE_F", b["FSIZE_F"])
    # PAGES block: page W/H, BCOL/FCOL/FSIZE if present
    for b in parse_blocks(lines, "PAGES"):
        if "W" in b: meta["PAGEW"] = coerce("PAGEW", b["W"])
        if "H" in b: meta["PAGEH"] = coerce("PAGEH", b["H"])
        # keep page default FSIZE/FCOL/BCOL if you want them later
        if "FSIZE" in b and "FSIZE_F" not in meta:
            meta["FSIZE_F"] = coerce("FSIZE_F", b["FSIZE"])
        break
    # Sensible fallbacks
    meta.setdefault("FONTNAME_F", "Arial")
    meta.setdefault("FSIZE_F", 9.0)
    meta.setdefault("PAGEW", 600)
    meta.setdefault("PAGEH", 600)
    return meta

def parse_tpccp(lines):
    controls = []
    for b in parse_blocks(lines, "TPCCP"):
        d = {}
        # pass-through keys with typing
        for k in ("ID","CID","PARENT","STYLE","TEXT","HELPTEXT"):
            if k in b: d[k] = b[k]
        for k in ("CTYPE","X","Y","H","W","TAB","BCOL","FSIZE","ZINDEX"):
            if k in b:
                d[k] = coerce(k, b[k])
        # flags
        for k in ("DISPLAY","READONLY","VISIBLE"):
            if k in b:
                d[k] = coerce(k, b[k])
        # Some files have empty <PARENT/> — normalize to page id if missing
        if not d.get("PARENT"):
            d["PARENT"] = b.get("PARENT") or "PG23271"
        controls.append(d)
    return controls

def parse_tpcp(lines):
    props = []
    for b in parse_blocks(lines, "TPcP"):
        d = {}
        # Only keep useful properties we apply in the renderer (ALIGN/ITEMS/DEFAULT etc.)
        keep = {"ID","ALIGN","ITEMS","DEFAULT","MAX","BTYPE","TEXTMODE","TOOLBARMODE",
                "DIRECTION","BLANK","SELECT","FUNCTIONORDER","RUNJAVASCRIPT"}
        for k,v in b.items():
            if k in keep:
                d[k] = v
        if "ID" in d:
            props.append(d)
    return props

def parse_efx(path):
    text = Path(path).read_text(encoding="utf-8", errors="ignore")
    lines = text.splitlines()
    data = {
        "meta": parse_meta(lines),
        "tpccp": parse_tpccp(lines),
        "tpcp": parse_tpcp(lines),
    }
    return data

# ── Flat-per-object, pretty array writer ───────────────────────────────────────
def flat_objects_lines(lst, item_indent=4):
    lines = []
    for d in lst:
        # Keep key order stable-ish
        order = ["ID","CID","CTYPE","PARENT","TEXT","X","Y","FSIZE","BCOL","H","W",
                 "TAB","DISPLAY","READONLY","VISIBLE","STYLE","ZINDEX","HELPTEXT"]
        items = []
        for k in order:
            if k in d:
                items.append(f'"{k}":{json.dumps(d[k], ensure_ascii=False)}')
        # include any extra keys not in the preferred order
        for k in d:
            if k not in order:
                items.append(f'"{k}":{json.dumps(d[k], ensure_ascii=False)}')
        inner = ", ".join(items)
        lines.append(" " * item_indent + "{" + inner + "}")
    return lines

def render_output(data: dict) -> str:
    meta_line = f'  "meta": {json.dumps(data["meta"], ensure_ascii=False)},'
    tpccp_lines = flat_objects_lines(data["tpccp"], item_indent=4)
    tpcp_lines  = flat_objects_lines(data["tpcp"],  item_indent=4)
    tpccp_block = "  \"tpccp\": [\n" + ",\n".join(tpccp_lines) + "\n  ],"
    tpcp_block  = "  \"tpcp\": [\n"  + ",\n".join(tpcp_lines)  + "\n  ]"
    return "{\n" + "\n".join([meta_line, tpccp_block, tpcp_block]) + "\n}\n"

def main():
    if len(sys.argv) < 2:
        print("Usage: python efx_to_json.py MF_Advanced_Army_Grading_Concept.efx > eform-data.json")
        sys.exit(1)
    data = parse_efx(sys.argv[1])
    print(render_output(data), end="")

if __name__ == "__main__":
    main()
